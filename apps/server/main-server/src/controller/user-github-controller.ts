...
export const getAllGithubRepos = asyncHandler(async (req, resp) => {
  ...
  resp.status(200).json(new ApiResponse('Github repos fetched successfully', 200, repos));
});
...
export const getSingleGithubRepo = asyncHandler(async (req, resp) => {
  ...
  resp.status(200).json(new ApiResponse('Github repo fetched successfully', 200, repo));
});
